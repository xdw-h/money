...
<template>
  ...
  <Teleport to="body"><div v-if="open" class="emoji-overlay" @click.stop="onOverlayClick"><section class="emoji-sheet" role="dialog" aria-modal="true" :aria-label="`选择${label}`"><header><div><strong>选择{{ label }}</strong><small>常用记账图标</small></div><button type="button" aria-label="关闭图标选择器" @click="close">×</button></header><div class="icon-groups"><section v-for="group in iconGroups" :key="group.name"><h3>{{ group.name }}</h3><div><button v-for="icon in group.icons" :key="icon" type="button" :class="{ selected: modelValue === icon }" :aria-label="`选择图标 ${icon}`" @click="choose(icon)">{{ icon }}<b v-if="modelValue === icon">✓</b></button></div></section></div><button class="cancel-picker" type="button" @click="close">取消</button></section></div></Teleport>
</template>
...
.emoji-overlay{z-index:300;overscroll-behavior:none;touch-action:pan-y}.emoji-sheet{height:min(76dvh,650px);display:grid;grid-template-rows:auto minmax(0,1fr) auto;overscroll-behavior:none}.icon-groups{min-height:0;max-height:none;overflow-y:auto;overscroll-behavior:contain;touch-action:pan-y;-webkit-overflow-scrolling:touch}
</style>
