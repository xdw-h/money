...
useBodyScrollLock(computed(() => subcategorySheetOpen.value || addingCategory.value || managingCategories.value || noteSheetOpen.value || dateTimeSheetOpen.value))
...
<template>
  <form class="record-editor" @submit.prevent="submit">
    ...
    <section class="entry-tools"><button class="note-trigger" type="button" @click="noteSheetOpen = true"><span>✎</span><small>{{ note || '添加备注' }}</small></button><button class="date-field" type="button" aria-label="选择记账时间" @click="dateTimeSheetOpen=true"><span>◷</span><small>{{ occurredAtLabel }}</small></button><ImageUploader :existing-images="initialImages" @update:files="files = $event" @update:retained-image-ids="retainedImageIds = $event" /></section>
    <DateTimePickerSheet v-if="dateTimeSheetOpen" :model-value="occurredAt" title="选择记账时间" @close="dateTimeSheetOpen=false" @select="selectOccurredAt" />
...
.form-card{overflow:hidden;border:1px solid var(--border);border-radius:16px;background:var(--surface)}.note-field,.date-field{min-height:46px;padding:0 13px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--border);font-size:12px}.date-field{border-bottom:0}.note-field textarea{max-width:70%;margin:0;padding:0;border:0;resize:none;background:transparent;text-align:right;color:var(--muted)}.receipt-card{padding:12px;border:1px solid var(--border);border-radius:16px;background:var(--surface)}.receipt-card>div{display:flex;justify-content:space-between;margin-bottom:10px;font-size:12px}.receipt-card small{color:var(--muted)}
...
.category-grid,.category-grid.income-grid{grid-template-columns:repeat(5,minmax(0,1fr));grid-template-rows:none;grid-auto-flow:row;grid-auto-columns:auto;grid-auto-rows:70px;align-content:start;overflow-x:hidden;overflow-y:auto;overscroll-behavior-y:contain}
.date-field small{min-width:0;overflow:hidden;color:var(--muted);font-size:9px;white-space:nowrap;text-overflow:ellipsis}
</style>
