...
<template>
  <div v-if="open && current" class="gallery" role="dialog" aria-modal="true" aria-label="图片预览">
    <header><button aria-label="关闭预览" @click="emit('close')">×</button><span>{{ index + 1 }} / {{ images.length }}</span><button v-if="deletable" aria-label="删除当前图片" @click="emit('delete', current.id)">删除</button><i v-else aria-hidden="true" /></header>
