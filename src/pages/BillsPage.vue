...
const cycleRange = computed(() => billingCycleRange(anchor.value, cycleAnchorDate.value))
...
const summary = computed(() => summarize(records.value, { mode: mode.value, anchor: anchor.value, timeZone, cycleAnchorDate: cycleAnchorDate.value }))
const trend = computed(() => trendSeries(records.value, { mode: mode.value, type: type.value, anchor: anchor.value, timeZone, cycleAnchorDate: cycleAnchorDate.value }))
const ranking = computed(() => categoryRanking(records.value, { mode: mode.value, type: type.value, anchor: anchor.value, timeZone, cycleAnchorDate: cycleAnchorDate.value }))
const filtered = computed(() => filterPeriod(records.value, { mode: mode.value, type: type.value, anchor: anchor.value, timeZone, cycleAnchorDate: cycleAnchorDate.value }))
...
<template>
  <main class="page bills-page">
    ...
    <div class="period-input"><input v-if="mode === 'month'" v-model="monthValue" type="month" aria-label="选择月份" /><input v-else v-model.number="yearValue" type="number" min="2000" max="2100" aria-label="选择年份" /><small v-if="mode === 'month'">本期：{{ cycleRange.start }} 至 {{ cycleRange.endInclusive }}</small></div>
