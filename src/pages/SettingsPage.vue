...
import { activeLedgerId, addLedger, clearLedgerCycleStartDate, deleteLedger, ledgerItems, loadLedgers, setActiveLedger, setLedgerCycleStartDate, updateLedger } from '../features/ledgers/ledgerStore'
...
async function resetMonthlyCycle(id: string, name: string, month: string) { try { await clearLedgerCycleStartDate(id, month); message.value = `“${name}”${month}月已恢复默认起始日` } catch (reason) { error.value = reason instanceof Error ? reason.message : '恢复默认失败' } }
...
<template>
  <main class="page settings-page"><header><h1>设置</h1><small>管理应用与本地数据</small></header>
    <h2>多账本</h2><section class="ledger-list"><div v-for="ledger in ledgerItems" :key="ledger.id" :class="{active:activeLedgerId===ledger.id}"><button class="ledger-main" type="button" @click="setActiveLedger(ledger.id)"><i>{{ ledger.icon }}</i><span><b>{{ ledger.name }}</b><small>{{ activeLedgerId===ledger.id ? '当前账本' : '切换到账本' }} · 默认每月{{ Number(ledger.cycleAnchorDate.slice(8,10)) }}日起算</small></span><em>{{ activeLedgerId===ledger.id ? '✓' : '›' }}</em></button><label class="cycle-start default-cycle"><span>默认起始日</span><input type="date" :value="ledger.cycleAnchorDate" :aria-label="`${ledger.name}默认账期起始日期`" @change="changeCycleAnchorDate(ledger.id,ledger.name,ledger.icon,($event.target as HTMLInputElement).value)" /></label><div class="monthly-cycle"><label><span>单独设置月份</span><input type="month" :value="selectedCycleMonth(ledger.id)" :aria-label="`${ledger.name}选择账期月份`" @change="setSelectedCycleMonth(ledger.id,($event.target as HTMLInputElement).value)" /></label><label><span>该月起始日</span><input type="date" :min="`${selectedCycleMonth(ledger.id)}-01`" :max="monthLastDate(selectedCycleMonth(ledger.id))" :value="ledger.cycleStartDates?.[selectedCycleMonth(ledger.id)] || defaultCycleDate(selectedCycleMonth(ledger.id),ledger.cycleAnchorDate)" :aria-label="`${ledger.name}${selectedCycleMonth(ledger.id)}起始日期`" @change="changeMonthlyCycle(ledger.id,ledger.name,selectedCycleMonth(ledger.id),($event.target as HTMLInputElement).value)" /></label><button v-if="ledger.cycleStartDates?.[selectedCycleMonth(ledger.id)]" type="button" @click="resetMonthlyCycle(ledger.id,ledger.name,selectedCycleMonth(ledger.id))">恢复默认</button><small v-else>当前沿用默认</small></div><button type="button" aria-label="修改账本" @click="renameLedger(ledger.id,ledger.name,ledger.icon)">修改</button><button class="remove-ledger" type="button" aria-label="删除账本" @click="removeLedger(ledger.id,ledger.name)">删除</button></div><button class="add-ledger" type="button" @click="createLedger">＋ 新增账本</button></section>
