...
<template>
  <main class="page home-page">
    ...
    <div class="recent-controls"><div><button v-for="item in [{ id: '3', label: '近3天' }, { id: '7', label: '近7天' }, { id: 'month', label: '本月' }]" :key="item.id" type="button" :aria-pressed="recentRange === item.id" @click="recentRange = item.id as RecentRange">{{ item.label }}</button></div><button class="sort-button" type="button" @click="recentSort = recentSort === 'time' ? 'amount' : 'time'"><span>{{ recentSort === 'time' ? '按时间' : '按金额' }}</span><i aria-hidden="true">⇅</i></button></div>
...
.recent-controls .sort-button{display:inline-flex;align-items:center;justify-content:center;gap:2px;line-height:1}.sort-button span{display:block}.sort-button i{width:10px;height:10px;display:grid;place-items:center;transform:translateY(0);font-size:10px;font-style:normal;line-height:1}
</style>
